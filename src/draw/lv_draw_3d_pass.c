...
static void destroy_fbo_resources(lv_evgpu_3d_pass_fbo_t * fbo);
...
{
    if(!lv_evgpu_3d_pass_layer_is(layer)) return NULL;
...
void lv_evgpu_3d_pass_layer_destroy(lv_layer_t * pass_layer, lv_display_t * disp)
{
    ...
    if(ud != NULL) {
        lv_evgpu_3d_pass_destroy_fbo(&ud->pass);
        lv_free(ud);
...
{
    if(!lv_evgpu_3d_pass_layer_is(pass_layer) || camera == NULL) return;
...
bool lv_evgpu_3d_pass_bind_fbo(lv_layer_t * pass_layer, int32_t * w, int32_t * h, lv_evgpu_3d_pass_fbo_t ** fbo)
{
    ...
    *h = lv_area_get_height(&pass_layer->buf_area);
    if(lv_evgpu_3d_pass_ensure(pass, *w, *h) != LV_RESULT_OK) return false;
