...
static inline void apply_recolor(EVGRpaint * paint, const lv_draw_image_dsc_t * dsc);
                          ...
                          int image_handle)
{
    ...
    if(image_handle < 0) {
        ...
        if(dsc->tile) {
#ifdef EVGR_GLES2_IMPLEMENTATION
...
#endif
            {
                ...
                image_flags |= EVGR_IMAGE_REPEATX | EVGR_IMAGE_REPEATY;
            }
        }

        image_handle = lv_evgpu_image_cache_get_handle(u, dsc->src,
                                                        image_flags, NULL);
    }
    ...
    if(dsc->tile) {
        ...
        if(lv_evgpu_tex_draw_image(u, dsc, coords, rect_w, rect_h, image_handle, &clip_area, &draw_matrix)) {
            LV_PROFILER_DRAW_END;
            return;
        }
    }

    lv_evgpu_transform(u->evgr, &image_matrix);

    evgrBeginPath(u->evgr);
    lv_evgpu_path_append_rect(u->evgr, 0, 0, rect_w, rect_h, dsc->clip_radius);

    EVGRpaint paint = evgrImagePattern(u->evgr, img_ofs_x, img_ofs_y, img_w, img_h, 0, image_handle,
...
static inline void apply_recolor(EVGRpaint * paint, const lv_draw_image_dsc_t * dsc)
{
    ...
    paint->recolor = evgrRGBA(dsc->recolor.red,
                             dsc->recolor.green,
                             dsc->recolor.blue,
                             dsc->recolor_opa);
}
    ...
    while(tile_area.y1 <= coords->y2) {
        while(tile_area.x1 <= coords->x2) {
            ...
            if(lv_area_intersect(&clipped_img_area, &tile_area, coords)) {
                evgrBeginPath(u->evgr);
                lv_evgpu_path_append_rect(u->evgr, img_ofs_x, img_ofs_y, img_w, img_h, 0);
                EVGRpaint paint = evgrImagePattern(u->evgr, img_ofs_x, img_ofs_y, img_w, img_h, 0, image_handle,
                                                 dsc->opa / (float)LV_OPA_COVER);
                apply_recolor(&paint, dsc);
                evgrFillPaint(u->evgr, paint);
