...
bool lv_evgpu_grad_to_paint(EVGRcontext * ctx, const lv_vector_gradient_t * grad, EVGRpaint * paint)
{
    ...
    const EVGRcolor icol = lv_evgpu_color_convert(grad->stops[0].color, grad->stops[0].opa);
    const EVGRcolor ocol = lv_evgpu_color_convert(grad->stops[1].color, grad->stops[1].opa);

    switch(grad->style) {
        case LV_VECTOR_GRADIENT_STYLE_LINEAR:
            *paint = evgrLinearGradient(ctx, grad->x1, grad->y1, grad->x2, grad->y2, icol, ocol);
            break;
        ...
        case LV_VECTOR_GRADIENT_STYLE_RADIAL: {
                ...
                *paint = evgrRadialGradient(ctx, grad->cx, grad->cy, inr, outr, icol, ocol);
            }
            break;
    ...
    enum EVGRcompositeOperation composite_operation)
{
    ...
    EVGRpaint paint;
    if(!lv_evgpu_grad_to_paint(ctx, grad, &paint)) {
        LV_PROFILER_DRAW_END;
        return;
    }
    ...
    enum EVGRcompositeOperation composite_operation)
{
    ...
    lv_evgpu_draw_grad(ctx, &grad, winding, composite_operation);
}
