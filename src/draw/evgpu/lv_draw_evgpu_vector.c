...
#if (LV_USE_DRAW_EVGPU) && LV_USE_VECTOR_GRAPHIC
...
static void lv_path_to_evgr(EVGRcontext * ctx, const lv_vector_path_t * src, lv_fpoint_t * offset);
static void lv_path_stroke_dashed(EVGRcontext * ctx, const lv_vector_path_t * src,
                                  const lv_vector_stroke_dsc_t * stroke_dsc);
static enum EVGRcompositeOperation lv_blend_to_evgr(lv_vector_blend_t blend);
static enum EVGRwinding lv_fill_to_evgr(lv_vector_fill_t fill_rule);
static int lv_stroke_cap_to_evgr(lv_vector_stroke_cap_t cap);
static int lv_stroke_join_to_evgr(lv_vector_stroke_join_t join);
...
void lv_draw_evgpu_vector(lv_draw_task_t * t, const lv_draw_vector_dsc_t * dsc)
{
    ...
    evgrGlobalAlpha(u->evgr, t->opa / (float)LV_OPA_COVER);
...
static EVGRcolor lv_color32_to_evgr(lv_color32_t color, lv_opa_t opa)
{
    uint8_t a = LV_UDIV255(color.alpha * opa);
    return evgrRGBA(color.red, color.green, color.blue, a);
}

static void draw_fill(lv_draw_evgpu_unit_t * u, const lv_vector_fill_dsc_t * fill_dsc, const lv_fpoint_t * offset,
                      enum EVGRcompositeOperation comp_op)
{
    LV_PROFILER_DRAW_BEGIN;

    const enum EVGRwinding winding = lv_fill_to_evgr(fill_dsc->fill_rule);

    lv_evgpu_transform(u->evgr, &fill_dsc->matrix);

    switch(fill_dsc->style) {
        case LV_VECTOR_DRAW_STYLE_SOLID: {
                lv_evgpu_fill(u->evgr, winding, comp_op, lv_color32_to_evgr(fill_dsc->color, fill_dsc->opa));
            }
            break;
        case LV_VECTOR_DRAW_STYLE_PATTERN: {
                const lv_draw_image_dsc_t * img_dsc = &fill_dsc->img_dsc;
                lv_image_header_t header;
                int image_handle = lv_evgpu_image_cache_get_handle(u, img_dsc->src, 0, &header);
                if(image_handle < 0) {
                    LV_LOG_WARN("EVGPU vector pattern: image cache miss");
                    LV_PROFILER_DRAW_END;
                    return;
                }
                ...
                evgrPathWinding(u->evgr, winding);
                evgrGlobalCompositeOperation(u->evgr, comp_op);

                EVGRpaint paint = evgrImagePattern(u->evgr, offset_x, offset_y, (float)header.w, (float)header.h, 0.0f,
                ...
                if(!pattern_logged) {
                    pattern_logged = true;
                    LV_LOG_INFO("EVGPU vector pattern fill ready (%ux%u)", (unsigned)header.w, (unsigned)header.h);
                }
            }
            break;
        case LV_VECTOR_DRAW_STYLE_GRADIENT: {
                lv_evgpu_draw_grad(u->evgr, &fill_dsc->gradient, winding, comp_op);
            }
            break;
        default:
...
static bool draw_stroke_prepare(lv_draw_evgpu_unit_t * u, const lv_vector_stroke_dsc_t * stroke_dsc)
{
    lv_evgpu_transform(u->evgr, &stroke_dsc->matrix);

    evgrStrokeColor(u->evgr, lv_color32_to_evgr(stroke_dsc->color, stroke_dsc->opa));
    evgrStrokeWidth(u->evgr, stroke_dsc->width);
    evgrLineCap(u->evgr, lv_stroke_cap_to_evgr(stroke_dsc->cap));
    evgrLineJoin(u->evgr, lv_stroke_join_to_evgr(stroke_dsc->join));
    evgrMiterLimit(u->evgr, stroke_dsc->miter_limit > 0 ? stroke_dsc->miter_limit : 4.0f);
    ...
    switch(stroke_dsc->style) {
        ...
        case LV_VECTOR_DRAW_STYLE_GRADIENT: {
                EVGRpaint paint;
                if(!lv_evgpu_grad_to_paint(u->evgr, &stroke_dsc->gradient, &paint)) {
                    return false;
                }
                evgrStrokePaint(u->evgr, paint);
            }
            break;
...
static void draw_stroke(lv_draw_evgpu_unit_t * u, const lv_vector_stroke_dsc_t * stroke_dsc)
{
    LV_PROFILER_DRAW_BEGIN;
...
static void draw_stroke_dashed(lv_draw_evgpu_unit_t * u, const lv_vector_path_t * path,
                               const lv_vector_stroke_dsc_t * stroke_dsc)
{
    LV_PROFILER_DRAW_BEGIN;
    ...
    lv_path_stroke_dashed(u->evgr, path, stroke_dsc);
...
{
    LV_PROFILER_DRAW_BEGIN;
    lv_draw_evgpu_unit_t * u = ctx;

    /* clear area */
    if(!path) {
        EVGRcolor c = lv_color32_to_evgr(dsc->fill_dsc.color, dsc->fill_dsc.opa);
        evgrBeginPath(u->evgr);
        lv_evgpu_path_append_area(u->evgr, &dsc->scissor_area);
        lv_evgpu_fill(u->evgr, EVGR_CCW, EVGR_COPY, c);
        LV_PROFILER_DRAW_END;
        return;
    }
    ...
    evgrSave(u->evgr);
    lv_evgpu_transform(u->evgr, &dsc->matrix);

    lv_fpoint_t offset = {0, 0};
    lv_path_to_evgr(u->evgr, path, &offset);

    lv_evgpu_set_clip_area(u->evgr, &dsc->scissor_area);

    const enum EVGRcompositeOperation comp_op = lv_blend_to_evgr(dsc->blend_mode);
    evgrGlobalCompositeOperation(u->evgr, comp_op);
    ...
    for(uint32_t i = 0; i < op_size; i++) {
        switch(ops[i]) {
            case LV_VECTOR_PATH_OP_MOVE_TO: {
                    evgrMoveTo(ctx, point->x, point->y);
                    CMP_BOUNDS(point);
                    point++;
                }
                break;
            case LV_VECTOR_PATH_OP_LINE_TO: {
                    evgrLineTo(ctx, point->x, point->y);
                    CMP_BOUNDS(point);
                    point++;
                }
                break;
            case LV_VECTOR_PATH_OP_QUAD_TO: {
                    evgrQuadTo(ctx, point[0].x, point[0].y, point[1].x, point[1].y);
                    CMP_BOUNDS(&point[0]);
                    CMP_BOUNDS(&point[1]);
                    point += 2;
                }
            ...
            case LV_VECTOR_PATH_OP_CUBIC_TO: {
                    evgrBezierTo(ctx, point[0].x, point[0].y, point[1].x, point[1].y, point[2].x, point[2].y);
                    CMP_BOUNDS(&point[0]);
                    CMP_BOUNDS(&point[1]);
                    CMP_BOUNDS(&point[2]);
                    point += 3;
...
static enum EVGRcompositeOperation lv_blend_to_evgr(lv_vector_blend_t blend)
{
    switch(blend) {
        case LV_VECTOR_BLEND_SRC_OVER:
            return EVGR_SOURCE_OVER;
        case LV_VECTOR_BLEND_SRC_IN:
            return EVGR_SOURCE_IN;
        case LV_VECTOR_BLEND_DST_OVER:
            return EVGR_DESTINATION_OVER;
        case LV_VECTOR_BLEND_DST_IN:
            return EVGR_DESTINATION_IN;
...
#define EVGPU_DASH_FLAT_MAX 512
...
static void dash_polyline(EVGRcontext * ctx, const lv_fpoint_t * pts, uint32_t pt_count,
                                  ...
                                  const lv_vector_stroke_dsc_t * stroke_dsc)
{
    ...
    lv_fpoint_t flat[EVGPU_DASH_FLAT_MAX];
    ...
    for(uint32_t i = 0; i < op_size; i++) {
        switch(ops[i]) {
            ...
            case LV_VECTOR_PATH_OP_LINE_TO:
                if(!has_cur) break;
                cur = *point;
                if(flat_count < EVGPU_DASH_FLAT_MAX) flat[flat_count++] = cur;
                point++;
                break;

            case LV_VECTOR_PATH_OP_QUAD_TO:
                if(!has_cur) break;
                flatten_quad(&cur, &point[0], &point[1], flat, &flat_count, EVGPU_DASH_FLAT_MAX);
                cur = point[1];
                point += 2;
                break;
            ...
            case LV_VECTOR_PATH_OP_CUBIC_TO:
                if(!has_cur) break;
                flatten_cubic(&cur, &point[0], &point[1], &point[2], flat, &flat_count, EVGPU_DASH_FLAT_MAX);
                cur = point[2];
                point += 3;
                break;

            case LV_VECTOR_PATH_OP_CLOSE:
                if(has_cur && flat_count < EVGPU_DASH_FLAT_MAX) {
