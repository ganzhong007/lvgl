...
void lv_draw_evgpu_triangle(lv_draw_task_t * t, const lv_draw_triangle_dsc_t * dsc)
{
    ...
    evgrMoveTo(u->evgr, dsc->p[0].x, dsc->p[0].y);
    evgrLineTo(u->evgr, dsc->p[1].x, dsc->p[1].y);
    evgrLineTo(u->evgr, dsc->p[2].x, dsc->p[2].y);
    evgrClosePath(u->evgr);

    if(dsc->grad.dir != LV_GRAD_DIR_NONE) {
#if LV_USE_VECTOR_GRAPHIC
        lv_evgpu_draw_grad_helper(u->evgr, &tri_area, &dsc->grad, EVGR_CCW, EVGR_SOURCE_OVER);
#else
