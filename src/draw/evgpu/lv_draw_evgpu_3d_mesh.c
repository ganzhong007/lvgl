...
void lv_draw_evgpu_3d_mesh_init(void)
{
    ...
    if(s_mesh_ready) {
        ...
        LV_LOG_INFO("EVGPU 3D mesh ready (MESH pass shader)");
    }

    if(!s_phong_ready) {
        s_phong_ready = phong_shader_init(&s_phong_prog, s_phong_locs, 12);
        if(s_phong_ready) {
            LV_LOG_INFO("EVGPU 3D phong ready (MESH phong lights)");
        }
    }
}
...
void lv_draw_evgpu_3d_mesh(lv_draw_task_t * t, const lv_draw_3d_mesh_dsc_t * dsc)
{
    ...
    if(!s_mesh_ready) lv_draw_evgpu_3d_mesh_init();
