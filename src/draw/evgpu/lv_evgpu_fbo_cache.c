...
#define LV_EVGPU_FBO_CACHE_CNT 4
...
typedef struct {
    ...
    int height;
    int flags;
    enum EVGRtexture format;

    /* value */
    struct EVGRLUframebuffer * fbo;
} fbo_item_t;
...
void lv_evgpu_fbo_cache_init(lv_draw_evgpu_unit_t * u)
{
    LV_ASSERT_NULL(u);
    ...
    u->fbo_cache = lv_cache_create(&lv_cache_class_lru_ll_count, sizeof(fbo_item_t), LV_EVGPU_FBO_CACHE_CNT, ops);
    lv_cache_set_name(u->fbo_cache, "EVGPU_FBO");
}

void lv_evgpu_fbo_cache_deinit(lv_draw_evgpu_unit_t * u)
{
    LV_ASSERT_NULL(u);
...
struct _lv_cache_entry_t * lv_evgpu_fbo_cache_get(lv_draw_evgpu_unit_t * u, int width, int height, int flags,
                                                   int format)
{
...
struct EVGRLUframebuffer * lv_evgpu_fbo_cache_entry_to_fb(struct _lv_cache_entry_t * entry)
{
    LV_ASSERT_NULL(entry);
...
static bool fbo_create_cb(fbo_item_t * item, void * user_data)
{
    ...
    item->fbo = evgrluCreateFramebuffer(item->u->evgr, item->width, item->height, item->flags, item->format);
    if(!item->fbo) {
...
static void fbo_free_cb(fbo_item_t * item, void * user_data)
{
    ...
    evgrluDeleteFramebuffer(item->fbo);
