...
#include "lv_draw_evgpu_private.h"
    ...
    #define LV_EVGPU_SHADER_HAS_GL 1
#else
    #define LV_EVGPU_SHADER_HAS_GL 0
#endif
...
#if LV_EVGPU_SHADER_HAS_GL

static const char evgpu_vert_src[] =
...
static const char evgpu_frag_src[] =
    "precision mediump float;\n"
    "varying vec2 v_pos;\n"
    "uniform vec4 u_rect;\n"
...
static const char evgpu_grad_vert_src[] =
    "precision mediump float;\n"
    "attribute vec2 a_pos;\n"
    "uniform mat3 u_matrix;\n"
    "uniform vec2 u_view_size;\n"
...
static const char evgpu_grad_frag_src[] =
    "precision mediump float;\n"
    "varying vec2 v_pos;\n"
    "uniform int u_dir;\n"
...
static const char evgpu_tex_vert_src[] =
    "precision mediump float;\n"
    "attribute vec2 a_pos;\n"
    "attribute vec2 a_uv;\n"
    "uniform mat3 u_matrix;\n"
...
static const char evgpu_tex_frag_src[] =
    "precision mediump float;\n"
    "varying vec2 v_uv;\n"
...
void lv_evgpu_shader_init(lv_draw_evgpu_unit_t * unit, lv_evgpu_context_t * ctx, lv_evgpu_shader_t * shader)
{
    LV_ASSERT_NULL(unit);
    LV_ASSERT_NULL(ctx);
...
#if LV_EVGPU_SHADER_HAS_GL
    GLuint vert = compile_shader(GL_VERTEX_SHADER, evgpu_vert_src);
    GLuint frag = compile_shader(GL_FRAGMENT_SHADER, evgpu_frag_src);
    if(vert == 0 || frag == 0) {
        if(vert) glDeleteShader(vert);
        if(frag) glDeleteShader(frag);
        LV_LOG_WARN("EVGPU shader: compile failed");
        return;
    }
    ...
    if(shader->solid_program == 0) {
        LV_LOG_WARN("EVGPU shader: link failed");
        return;
    }
    ...
    GLuint gvert = compile_shader(GL_VERTEX_SHADER, evgpu_grad_vert_src);
    GLuint gfrag = compile_shader(GL_FRAGMENT_SHADER, evgpu_grad_frag_src);
    if(gvert && gfrag) {
        ...
        if(shader->grad_program) {
            ...
            LV_LOG_INFO("EVGPU native grad shader ready (program=%u)", (unsigned)shader->grad_program);
        }
    }
    else {
        if(gvert) glDeleteShader(gvert);
        if(gfrag) glDeleteShader(gfrag);
        LV_LOG_WARN("EVGPU grad shader: compile failed");
    }

    GLuint tvert = compile_shader(GL_VERTEX_SHADER, evgpu_tex_vert_src);
    GLuint tfrag = compile_shader(GL_FRAGMENT_SHADER, evgpu_tex_frag_src);
    if(tvert && tfrag) {
        ...
        if(ok) {
            ...
            LV_LOG_INFO("EVGPU native tex shader ready (program=%u)", (unsigned)shader->tex_program);
        }
        else {
            char log[256];
            GLsizei len = 0;
            glGetProgramInfoLog(shader->tex_program, (GLsizei)sizeof(log), &len, log);
            LV_LOG_WARN("EVGPU tex shader link: %s", log);
        ...
        LV_LOG_WARN("EVGPU tex shader: compile failed");
    }

    /* Self-test: bind once so apitrace / logs can confirm native path */
    glUseProgram(shader->solid_program);
    lv_evgpu_context_set_bound_program(ctx, shader->solid_program);
    glUseProgram(0);
    lv_evgpu_context_set_bound_program(ctx, 0);

    shader->ready = true;
    LV_LOG_INFO("EVGPU native shader ready (program=%u)", (unsigned)shader->solid_program);
#else
    LV_UNUSED(unit);
    LV_UNUSED(ctx);
    LV_LOG_INFO("EVGPU native shader skipped (no EGL GLES2)");
#endif
}
...
bool lv_evgpu_shader_bind_solid(lv_evgpu_context_t * ctx, lv_evgpu_shader_t * shader)
{
    if(!lv_evgpu_shader_is_ready(shader)) return false;

#if LV_EVGPU_SHADER_HAS_GL
    glUseProgram(shader->solid_program);
    if(ctx) lv_evgpu_context_set_bound_program(ctx, shader->solid_program);
    return true;
#else
    LV_UNUSED(ctx);
    return false;
...
void lv_evgpu_shader_unbind(lv_evgpu_context_t * ctx)
{
#if LV_EVGPU_SHADER_HAS_GL
    glUseProgram(0);
    if(ctx) lv_evgpu_context_set_bound_program(ctx, 0);
#else
    LV_UNUSED(ctx);
#endif
}
...
bool lv_evgpu_shader_bind_grad(lv_evgpu_context_t * ctx, lv_evgpu_shader_t * shader)
{
    if(!lv_evgpu_shader_grad_is_ready(shader)) return false;

#if LV_EVGPU_SHADER_HAS_GL
    glUseProgram(shader->grad_program);
    if(ctx) lv_evgpu_context_set_bound_program(ctx, shader->grad_program);
    return true;
#else
    LV_UNUSED(ctx);
    return false;
...
bool lv_evgpu_shader_bind_tex(lv_evgpu_context_t * ctx, lv_evgpu_shader_t * shader)
{
    if(!lv_evgpu_shader_tex_is_ready(shader)) return false;

#if LV_EVGPU_SHADER_HAS_GL
    glUseProgram(shader->tex_program);
    if(ctx) lv_evgpu_context_set_bound_program(ctx, shader->tex_program);
    return true;
#else
    LV_UNUSED(ctx);
    return false;
...
static GLuint compile_shader(GLenum type, const char * src)
{
    ...
    if(!ok) {
        ...
        LV_LOG_WARN("EVGPU shader compile: %s", log);
        glDeleteShader(sh);
        return 0;
    }
...
static GLuint link_program(GLuint vert, GLuint frag)
{
    ...
    if(!ok) {
        ...
        LV_LOG_WARN("EVGPU shader link: %s", log);
