...
void lv_draw_evgpu_label_init(lv_draw_evgpu_unit_t * u)
{
    ...
    u->letter_cache = lv_cache_create(&lv_cache_class_lru_rb_count, sizeof(letter_item_t), LV_EVGR_LETTER_CACHE_CNT, ops);
    lv_cache_set_name(u->letter_cache, "EVGR_LETTER");
    ...
    u->label_text_cache = lv_malloc_zeroed(sizeof(lv_evgpu_label_text_cache_t));
    LV_ASSERT_MALLOC(u->label_text_cache);
    lv_evgpu_label_text_cache_init(u->label_text_cache);
    LV_LOG_INFO("EVGPU label text hash ready");
}
...
void lv_draw_evgpu_label(lv_draw_task_t * t, const lv_draw_label_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    if(t && t->draw_unit && dsc) {
        ...
        if(u->label_text_cache) {
            lv_evgpu_label_text_cache_touch(u->label_text_cache, dsc);
        }
    }
...
static void draw_letter_bitmap(lv_draw_task_t * t, const lv_draw_glyph_dsc_t * dsc, int image_handle)
{
    ...
    if(!dsc->rotation) {
        ...
        EVGRpaint paint = evgrImagePattern(u->evgr, x, y, w, h, 0, image_handle, 1.0f);
        paint.innerColor = paint.outerColor = evgrRGBA(dsc->color.red, dsc->color.green, dsc->color.blue, dsc->opa);

        evgrBeginPath(u->evgr);
        evgrRect(u->evgr, x, y, w, h);
...
static inline int letter_get_image_handle(lv_draw_evgpu_unit_t * u, lv_font_glyph_dsc_t * g_dsc)
{
    LV_PROFILER_DRAW_BEGIN;
...
static bool letter_create_cb(letter_item_t * item, void * user_data)
{
    ...
    lv_draw_buf_t * image_buf = lv_evgpu_reshape_global_image(item->u, LV_COLOR_FORMAT_A8, w, h);
    if(!image_buf) {
        LV_PROFILER_DRAW_END;
        return false;
    }
    ...
    LV_PROFILER_DRAW_BEGIN_TAG("evgrCreateImage");
    item->image_handle = evgrCreateImage(item->u->evgr, w, h, 0, EVGR_TEXTURE_ALPHA, lv_draw_buf_goto_xy(bitmap_draw_buf, 0,
                                                                                                     0));
    LV_PROFILER_DRAW_END_TAG("evgrCreateImage");
...
static void letter_free_cb(letter_item_t * item, void * user_data)
{
    ...
    LV_PROFILER_DRAW_BEGIN;
    LV_LOG_TRACE("image_handle: %d", item->image_handle);
    evgrDeleteImage(item->u->evgr, item->image_handle);
    item->image_handle = -1;
    LV_PROFILER_DRAW_END;
}
                           ...
                           lv_draw_fill_dsc_t * fill_draw_dsc, const lv_area_t * fill_area)
{
    ...
    if(glyph_draw_dsc) {
        switch(glyph_draw_dsc->format) {
            ...
            case LV_FONT_GLYPH_FORMAT_IMAGE: {
                    ...
                    lv_draw_evgpu_image(t, &image_dsc, glyph_draw_dsc->letter_coords, -1);
                }
                break;
            ...
            case LV_FONT_GLYPH_FORMAT_NONE: {
                    ...
                    lv_draw_evgpu_border(t, &border_draw_dsc, glyph_draw_dsc->bg_coords);
                }
                break;
    ...
    if(fill_draw_dsc && fill_area) {
        lv_draw_evgpu_fill(t, fill_draw_dsc, fill_area);
    }
}
