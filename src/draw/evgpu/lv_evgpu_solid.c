...
#include "lv_evgpu_utils.h"
    ...
    #define LV_EVGPU_SOLID_HAS_GL 1
#else
    #define LV_EVGPU_SOLID_HAS_GL 0
#endif

typedef struct {
    uint32_t vbo;
    bool ready;
#if LV_EVGPU_SOLID_HAS_GL
    int32_t loc_matrix;
    ...
    int32_t loc_color;
#endif
} lv_evgpu_solid_state_t;

static lv_evgpu_solid_state_t g_solid_state;
...
void lv_evgpu_solid_init(lv_draw_evgpu_unit_t * unit)
{
    lv_memzero(&g_solid_state, sizeof(g_solid_state));

#if LV_EVGPU_SOLID_HAS_GL
    if(!lv_evgpu_shader_is_ready(&unit->shader)) return;
    ...
    LV_LOG_INFO("EVGPU native solid fill ready (program=%u)", (unsigned)prog);
#else
    LV_UNUSED(unit);
#endif
}

void lv_evgpu_solid_deinit(lv_draw_evgpu_unit_t * unit)
{
    LV_UNUSED(unit);
#if LV_EVGPU_SOLID_HAS_GL
...
bool lv_evgpu_solid_fill_rect(lv_draw_evgpu_unit_t * unit,
                             ...
                             const lv_matrix_t * matrix)
{
...
#if LV_EVGPU_SOLID_HAS_GL
    if(!g_solid_state.ready || !lv_evgpu_shader_is_ready(&unit->shader)) return false;
    ...
    float xform[6];
    lv_evgpu_matrix_convert(xform, matrix);
    float mat3[9];
    ...
    lv_evgpu_shader_bind_solid(&unit->ctx, &unit->shader);
