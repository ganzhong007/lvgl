...
void lv_evgpu_image_cache_init(struct _lv_draw_evgpu_unit_t * u)
{
    ...
    u->image_cache = lv_cache_create(&lv_cache_class_lru_rb_count, sizeof(image_item_t), LV_EVGR_IMAGE_CACHE_CNT, ops);
    lv_cache_set_name(u->image_cache, "EVGR_IMAGE");
...
int lv_evgpu_image_cache_get_handle(struct _lv_draw_evgpu_unit_t * u,
                                     const void * src,
                                     int image_flags,
...
static bool image_create_cb(image_item_t * item, void * user_data)
{
    ...
    enum EVGRtexture evgr_tex_type = EVGR_TEXTURE_BGRA;

    /* Determine texture type and pixel size based on color format */
    switch(cf) {
        case LV_COLOR_FORMAT_A8:
            evgr_tex_type = EVGR_TEXTURE_ALPHA;
            break;

        case LV_COLOR_FORMAT_ARGB8888:
        case LV_COLOR_FORMAT_ARGB8888_PREMULTIPLIED:
            evgr_tex_type = EVGR_TEXTURE_BGRA;
            break;

        case LV_COLOR_FORMAT_XRGB8888:
            evgr_tex_type = EVGR_TEXTURE_BGRX;
            break;

        case LV_COLOR_FORMAT_RGB888:
            evgr_tex_type = EVGR_TEXTURE_BGR;
            break;

        case LV_COLOR_FORMAT_RGB565:
            evgr_tex_type = EVGR_TEXTURE_RGB565;
            break;
    ...
    if(stride == tight_stride) {
        ...
        lv_draw_buf_t * tmp_buf = lv_evgpu_reshape_global_image(item->u, cf, w, h);
        if(!tmp_buf) {
    ...
    if(cf == LV_COLOR_FORMAT_ARGB8888_PREMULTIPLIED
       || lv_draw_buf_has_flag(&item->src_buf, LV_IMAGE_FLAGS_PREMULTIPLIED)) {
        flags |= EVGR_IMAGE_PREMULTIPLIED;
    }

    LV_PROFILER_DRAW_BEGIN_TAG("evgrCreateImage");
    int image_handle = evgrCreateImage(item->u->evgr, w, h, flags, evgr_tex_type, data);
