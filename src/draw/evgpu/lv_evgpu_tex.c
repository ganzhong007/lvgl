...
#include "lv_evgpu_utils.h"
    ...
    #define LV_EVGPU_TEX_HAS_GL 1
    extern GLuint evgrlImageHandleGLES2(EVGRcontext * ctx, int image);
#else
    #define LV_EVGPU_TEX_HAS_GL 0
#endif

typedef struct {
    uint32_t vbo;
    bool ready;
#if LV_EVGPU_TEX_HAS_GL
    int32_t loc_matrix;
...
#endif
} lv_evgpu_tex_state_t;

static lv_evgpu_tex_state_t g_tex_state;
...
void lv_evgpu_tex_init(lv_draw_evgpu_unit_t * unit)
{
    lv_memzero(&g_tex_state, sizeof(g_tex_state));

#if LV_EVGPU_TEX_HAS_GL
    if(!lv_evgpu_shader_tex_is_ready(&unit->shader)) return;
    ...
    LV_LOG_INFO("EVGPU native texture draw ready (program=%u)", (unsigned)prog);
#else
    LV_UNUSED(unit);
#endif
}

void lv_evgpu_tex_deinit(lv_draw_evgpu_unit_t * unit)
{
    LV_UNUSED(unit);
#if LV_EVGPU_TEX_HAS_GL
...
bool lv_evgpu_tex_draw_image(lv_draw_evgpu_unit_t * unit,
                            ...
                            const lv_matrix_t * matrix)
{
...
#if LV_EVGPU_TEX_HAS_GL
    if(!g_tex_state.ready || !lv_evgpu_shader_tex_is_ready(&unit->shader)) return false;
    ...
    float xform[6];
    lv_evgpu_matrix_convert(xform, &draw_matrix);
    float mat3[9];
    lv_evgpu_xform_to_mat3(mat3, xform);
    ...
    const GLuint tex = evgrlImageHandleGLES2(unit->evgr, image_handle);
    ...
    lv_evgpu_shader_bind_tex(&unit->ctx, &unit->shader);
