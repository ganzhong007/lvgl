...
static void draw_execute(lv_draw_evgpuganesh_unit_t * u, lv_draw_task_t * t);

static void on_layer_changed(lv_layer_t * new_layer);
static void on_layer_readback(lv_draw_evgpuganesh_unit_t * u, lv_layer_t * layer);

void lv_draw_evgpuganesh_init(void)
{
    lv_draw_evgpuganesh_unit_t * unit = lv_draw_create_unit(sizeof(lv_draw_evgpuganesh_unit_t));
    ...
    unit->base_unit.name = "EVGPUGANESH";

    lv_evgpuganesh_gl_init(&unit->gl);

    LV_LOG_INFO("DrawUnitEVGPUGANESH ready (unit_id=%d)", EVGPUGANESH_UNIT_ID);
}
...
static void draw_execute(lv_draw_evgpuganesh_unit_t * u, lv_draw_task_t * t)
{
    ...
    lv_evgpuganesh_gl_set_scissor(t->clip_area.x1, t->clip_area.y1,
    ...
    LV_PORT_LAYER_TRACE("L3-EVGPUGANESH", "execute task type=%d at (%d,%d)-(%d,%d)",
                        t->type,
    ...
    switch(t->type) {
        case LV_DRAW_TASK_TYPE_FILL:
            lv_draw_evgpuganesh_fill(t, t->draw_dsc, &t->area);
            break;

        case LV_DRAW_TASK_TYPE_BORDER:
            lv_draw_evgpuganesh_border(t, t->draw_dsc, &t->area);
            break;

        case LV_DRAW_TASK_TYPE_BOX_SHADOW:
            lv_draw_evgpuganesh_box_shadow(t, t->draw_dsc, &t->area);
            break;

        case LV_DRAW_TASK_TYPE_LETTER:
            lv_draw_evgpuganesh_letter(t, t->draw_dsc, &t->area);
            break;

        case LV_DRAW_TASK_TYPE_LABEL:
            lv_draw_evgpuganesh_label(t, t->draw_dsc, &t->area);
            break;

        case LV_DRAW_TASK_TYPE_IMAGE:
            lv_draw_evgpuganesh_image(t, t->draw_dsc, &t->area, -1);
            break;

        case LV_DRAW_TASK_TYPE_LAYER:
            lv_draw_evgpuganesh_layer(t, t->draw_dsc, &t->area);
            break;

        case LV_DRAW_TASK_TYPE_LINE:
            lv_draw_line_iterate(t, t->draw_dsc, lv_draw_evgpuganesh_line);
            break;

        case LV_DRAW_TASK_TYPE_ARC:
            lv_draw_evgpuganesh_arc(t, t->draw_dsc, &t->area);
            break;

        case LV_DRAW_TASK_TYPE_TRIANGLE:
            lv_draw_evgpuganesh_triangle(t, t->draw_dsc);
            break;

        case LV_DRAW_TASK_TYPE_MASK_RECTANGLE:
            lv_draw_evgpuganesh_mask_rect(t, t->draw_dsc);
            break;

        case LV_DRAW_TASK_TYPE_BLUR:
            lv_draw_evgpuganesh_blur(t, t->draw_dsc, &t->area);
            break;

#if LV_USE_VECTOR_GRAPHIC
        case LV_DRAW_TASK_TYPE_VECTOR:
            lv_draw_evgpuganesh_vector(t, t->draw_dsc);
            break;
#endif
...
static void on_layer_changed(lv_layer_t * new_layer)
{
    ...
    lv_evgpuganesh_fbo_t * fbo = (lv_evgpuganesh_fbo_t *)new_layer->user_data;
...
static void on_layer_readback(lv_draw_evgpuganesh_unit_t * u, lv_layer_t * layer)
{
    ...
    lv_evgpuganesh_fbo_t * fbo = (lv_evgpuganesh_fbo_t *)layer->user_data;
...
static int32_t draw_dispatch(lv_draw_unit_t * draw_unit, lv_layer_t * layer)
{
    lv_draw_evgpuganesh_unit_t * u = (lv_draw_evgpuganesh_unit_t *)draw_unit;

    lv_draw_task_t * t = lv_draw_get_available_task(layer, NULL, EVGPUGANESH_UNIT_ID);
    if(!t || t->preferred_draw_unit_id != EVGPUGANESH_UNIT_ID) {
        lv_evgpuganesh_end_frame(u);
    ...
    if(!u->is_started) {
        ...
        lv_evgpuganesh_gl_set_projection(&u->gl, buf_w, buf_h);
...
static int32_t draw_evaluate(lv_draw_unit_t * draw_unit, lv_draw_task_t * task)
{
    switch(task->type) {
        ...
        case LV_DRAW_TASK_TYPE_BLUR:
            ...
            task->preferred_draw_unit_id = EVGPUGANESH_UNIT_ID;
            return 1;
...
static int32_t draw_delete(lv_draw_unit_t * draw_unit)
{
    lv_draw_evgpuganesh_unit_t * unit = (lv_draw_evgpuganesh_unit_t *)draw_unit;
    lv_evgpuganesh_gl_deinit(&unit->gl);
    return 0;
}

static void draw_event_cb(lv_event_t * e)
{
    lv_draw_evgpuganesh_unit_t * u = lv_event_get_current_target(e);
    lv_layer_t * layer = lv_event_get_param(e);

    switch(lv_event_get_code(e)) {
        case LV_EVENT_CANCEL:
            lv_evgpuganesh_clean_up(u);
            break;

        case LV_EVENT_CHILD_CREATED: {
                lv_evgpuganesh_fbo_t * fbo = lv_evgpuganesh_fbo_create(
        ...
        case LV_EVENT_CHILD_DELETED: {
                lv_evgpuganesh_fbo_t * fbo = (lv_evgpuganesh_fbo_t *)layer->user_data;
                if(fbo) {
                    lv_evgpuganesh_fbo_destroy(fbo);
