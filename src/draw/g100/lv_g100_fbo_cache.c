...
#define LV_G100_FBO_CACHE_CNT 4
...
void lv_g100_fbo_cache_init(lv_draw_g100_unit_t * u)
{
    LV_ASSERT_NULL(u);
    ...
    u->fbo_cache = lv_cache_create(&lv_cache_class_lru_ll_count, sizeof(fbo_item_t), LV_G100_FBO_CACHE_CNT, ops);
    lv_cache_set_name(u->fbo_cache, "G100_FBO");
}

void lv_g100_fbo_cache_deinit(lv_draw_g100_unit_t * u)
{
    LV_ASSERT_NULL(u);
...
struct _lv_cache_entry_t * lv_g100_fbo_cache_get(lv_draw_g100_unit_t * u, int width, int height, int flags,
                                                   int format)
{
