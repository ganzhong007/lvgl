...
#include "../../misc/lv_math.h"
    ...
    #define LV_G100_GRAD_HAS_GL 1
#else
    #define LV_G100_GRAD_HAS_GL 0
#endif
...
#define LV_G100_GRAD_MAX_SHADER_STOPS 8
...
#endif
} lv_g100_grad_state_t;

/**********************
 *  STATIC VARIABLES
 **********************/

static lv_g100_grad_state_t g_grad_state;
...
void lv_g100_grad_init(lv_draw_g100_unit_t * unit)
{
    lv_memzero(&g_grad_state, sizeof(g_grad_state));
    g_grad_state.unit = unit;

#if LV_G100_GRAD_HAS_GL
    if(!lv_g100_shader_grad_is_ready(&unit->shader)) return;
    ...
    LV_LOG_INFO("G100 native gradient ready (program=%u)", (unsigned)prog);
#else
    LV_UNUSED(unit);
#endif
}

void lv_g100_grad_deinit(lv_draw_g100_unit_t * unit)
{
    LV_UNUSED(unit);
#if LV_G100_GRAD_HAS_GL
    if(g_grad_state.vbo) {
...
bool lv_g100_grad_fill_rect(lv_draw_g100_unit_t * unit,
...
#if LV_G100_GRAD_HAS_GL
    if(!g_grad_state.ready || !lv_g100_shader_grad_is_ready(&unit->shader)) return false;

    lv_g100_nvg_flush_pending(unit);

    float colors[LV_G100_GRAD_MAX_SHADER_STOPS * 4];
    float fracs[LV_G100_GRAD_MAX_SHADER_STOPS];
...
static void pack_stops(const lv_grad_dsc_t * grad_dsc, float * colors, float * fracs, int32_t * count)
{
    const int32_t n = LV_MIN((int32_t)grad_dsc->stops_count, LV_G100_GRAD_MAX_SHADER_STOPS);
    *count = n;
