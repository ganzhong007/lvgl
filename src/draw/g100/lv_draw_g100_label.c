...
static inline int letter_get_image_handle(lv_draw_g100_unit_t * u, lv_font_glyph_dsc_t * g_dsc)
{
    LV_PROFILER_DRAW_BEGIN;
...
static bool letter_create_cb(letter_item_t * item, void * user_data)
{
    ...
    lv_draw_buf_t * image_buf = lv_g100_reshape_global_image(item->u, LV_COLOR_FORMAT_A8, w, h);
    if(!image_buf) {
        LV_PROFILER_DRAW_END;
        return false;
    }
                           ...
                           lv_draw_fill_dsc_t * fill_draw_dsc, const lv_area_t * fill_area)
{
    ...
    if(glyph_draw_dsc) {
        switch(glyph_draw_dsc->format) {
            ...
            case LV_FONT_GLYPH_FORMAT_IMAGE: {
                    ...
                    lv_draw_g100_image(t, &image_dsc, glyph_draw_dsc->letter_coords, -1);
                }
                break;
            ...
            case LV_FONT_GLYPH_FORMAT_NONE: {
                    ...
                    lv_draw_g100_border(t, &border_draw_dsc, glyph_draw_dsc->bg_coords);
                }
                break;
    ...
    if(fill_draw_dsc && fill_area) {
        lv_draw_g100_fill(t, fill_draw_dsc, fill_area);
    }
}
