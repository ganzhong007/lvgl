...
static uint32_t lv_g100_text_len_resolve(const char * text, uint32_t text_length)
{
    if(!text) return 0;
...
uint32_t lv_g100_text_hash_compute(const char * text, uint32_t text_length)
{
    if(!text) return 0;

    const uint32_t len = lv_g100_text_len_resolve(text, text_length);
...
void lv_g100_label_text_key_from_dsc(lv_g100_label_text_key_t * key, const lv_draw_label_dsc_t * dsc)
{
    LV_ASSERT_NULL(key);
    LV_ASSERT_NULL(dsc);
    ...
    key->font = dsc->font;
    key->text_len = lv_g100_text_len_resolve(dsc->text, dsc->text_length);
    key->text_hash = lv_g100_text_hash_compute(dsc->text, dsc->text_length);
...
bool lv_g100_label_text_key_equal(const lv_g100_label_text_key_t * a, const lv_g100_label_text_key_t * b)
{
    LV_ASSERT_NULL(a);
    LV_ASSERT_NULL(b);
...
void lv_g100_label_text_cache_init(lv_g100_label_text_cache_t * cache)
{
    LV_ASSERT_NULL(cache);
    lv_memzero(cache, sizeof(*cache));
}

bool lv_g100_label_text_cache_touch(lv_g100_label_text_cache_t * cache, const lv_draw_label_dsc_t * dsc)
{
    LV_ASSERT_NULL(cache);
    LV_ASSERT_NULL(dsc);

    lv_g100_label_text_key_t key;
    lv_g100_label_text_key_from_dsc(&key, dsc);

    if(cache->valid && lv_g100_label_text_key_equal(&cache->last_key, &key)) {
        cache->streak++;
        if(cache->streak == 1 || (cache->streak % 128) == 0) {
            LV_LOG_INFO("G100 label text hash hit (hash=0x%08x, streak=%u)", (unsigned)key.text_hash,
                        (unsigned)cache->streak);
        }
        return true;
    }

    LV_LOG_INFO("G100 label text hash miss (hash=0x%08x, len=%u, static=%u)", (unsigned)key.text_hash,
