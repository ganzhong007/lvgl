...
void lv_draw_g100_arc(lv_draw_task_t * t, const lv_draw_arc_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    if(nvg_math_is_equal(sweep_angle, 360)) {
        ...
        if(radius_in > 0) {
            ...
            lv_g100_path_append_arc(u->vg, cx, cy, radius_out, start_angle, sweep_angle, true);
        }
    ...
    if(dsc->img_src) {
        ...
        int image_handle = lv_g100_image_cache_get_handle(u, dsc->img_src, 0, &header);
        if(image_handle < 0) {
            LV_PROFILER_DRAW_END;
            return;
        }
        ...
        lv_g100_fill(u->vg, winding, NVG_SOURCE_OVER, lv_g100_color_convert(dsc->color, dsc->opa));
    }
