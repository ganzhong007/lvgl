...
{
#if LV_USE_DRAW_G100
    if(!lv_g100_3d_pass_layer_is(pass_layer)) return;
...
{
#if LV_USE_DRAW_G100
    if(!lv_g100_3d_pass_layer_is(pass_layer) || light == NULL) return false;
...
{
#if LV_USE_DRAW_G100
    if(!lv_g100_3d_pass_layer_is(pass_layer)) return 0;
    lv_3d_pass_layer_ud_t * ud = pass_layer->user_data;
