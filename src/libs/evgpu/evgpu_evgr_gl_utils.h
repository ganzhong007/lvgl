...
//
#ifndef EVGPU_EVGR_GL_UTILS_H
#define EVGPU_EVGR_GL_UTILS_H
...
typedef struct EVGRLUframebuffer EVGRLUframebuffer;

// Helper function to create GL frame buffer to render to.
// format: see EVGRtexture
void evgrluBindFramebuffer(EVGRLUframebuffer * fb);
EVGRLUframebuffer * evgrluCreateFramebuffer(EVGRcontext * ctx, int w, int h, int imageFlags, int format);
void evgrluDeleteFramebuffer(EVGRLUframebuffer * fb);

// ---- Blur utilities ----

// Quality hint for blur operations
enum EVGRLUblurQuality {
    EVGRLU_BLUR_QUALITY_SPEED  = 0,  // Fewer taps, faster
    EVGRLU_BLUR_QUALITY_NORMAL = 1,  // Full taps, better quality
};

// Parameters for evgrluBlurRegion
typedef struct EVGRLUblurParams {
    int radius;                     // Blur radius in pixels (clamped to internal max)
    int quality;                    // EVGRLUblurQuality
    EVGRcolor recolor;               // If recolor.a > 0, apply as tint (for drop shadows)
} EVGRLUblurParams;

// Opaque blur state handle (created/destroyed per draw unit lifetime)
typedef struct EVGRLUblurState EVGRLUblurState;

// Create/destroy blur state (shader program, VBO, scratch resources)
EVGRLUblurState * evgrluCreateBlurState(void);
void evgrluDeleteBlurState(EVGRLUblurState * state, EVGRcontext * ctx);
...
int evgrluBlurRegion(EVGRLUblurState * state, EVGRcontext * ctx, EVGRLUframebuffer * fb,
                    int x, int y, int w, int h,
                    const EVGRLUblurParams * params);

// Returns GL texture name for an EVGRLU framebuffer (0 if invalid).
unsigned int evgrluFramebufferGetTexture(EVGRLUframebuffer * fb);

#endif // EVGPU_EVGR_GL_UTILS_H

#ifdef EVGR_GL_IMPLEMENTATION

#if defined(EVGR_GL3) || defined(EVGR_GLES2) || defined(EVGR_GLES3)
    // FBO is core in OpenGL 3>.
    #define EVGR_FBO_VALID 1
#elif defined(EVGR_GL2)
...
EVGRLUframebuffer * evgrluCreateFramebuffer(EVGRcontext * ctx, int w, int h, int imageFlags, int format)
{
#ifdef EVGR_FBO_VALID
    GLint defFBO;
    GLint defaultRBO;
    EVGRLUframebuffer * fb = NULL;

    glGetIntegerv(GL_FRAMEBUFFER_BINDING, &defFBO);
    glGetIntegerv(GL_RENDERBUFFER_BINDING, &defaultRBO);

    fb = (EVGRLUframebuffer *)lv_malloc(sizeof(EVGRLUframebuffer));
    if(fb == NULL) goto error;
    lv_memzero(fb, sizeof(EVGRLUframebuffer));

    fb->image = evgrCreateImage(ctx, w, h, imageFlags | EVGR_IMAGE_FLIPY | EVGR_IMAGE_PREMULTIPLIED, format, NULL);

#if defined EVGR_GL2
    fb->texture = evgrlImageHandleGL2(ctx, fb->image);
#elif defined EVGR_GL3
    fb->texture = evgrlImageHandleGL3(ctx, fb->image);
#elif defined EVGR_GLES2
    fb->texture = evgrlImageHandleGLES2(ctx, fb->image);
#elif defined EVGR_GLES3
    fb->texture = evgrlImageHandleGLES3(ctx, fb->image);
#endif
...
error:
    glBindFramebuffer(GL_FRAMEBUFFER, defFBO);
    glBindRenderbuffer(GL_RENDERBUFFER, defaultRBO);
    evgrluDeleteFramebuffer(fb);
    return NULL;
#else
    EVGR_NOTUSED(ctx);
    EVGR_NOTUSED(w);
    EVGR_NOTUSED(h);
    EVGR_NOTUSED(imageFlags);
    EVGR_NOTUSED(format);
    return NULL;
#endif
}
...
void evgrluDeleteFramebuffer(EVGRLUframebuffer * fb)
{
    ...
    if(fb->image >= 0)
        evgrDeleteImage(fb->ctx, fb->image);
    fb->ctx = NULL;
    fb->fbo = 0;
    fb->rbo = 0;
    fb->texture = 0;
...
GLuint evgrluFramebufferGetTexture(EVGRLUframebuffer * fb)
{
#ifdef EVGR_FBO_VALID
...
#define EVGRLU_BLUR_MAX_TAPS   16
#define EVGRLU_BLUR_MAX_RADIUS 256

// GL3/GLES3 need VAO
#if defined(EVGR_GL3) || defined(EVGR_GLES3)
    #define EVGRLU_BLUR_NEEDS_VAO 1
#else
    #define EVGRLU_BLUR_NEEDS_VAO 0
#endif

// GLSL version prologues
#if defined(EVGR_GL3)
#define EVGRLU_BLUR_VS_HEADER \
    "#version 130\n" \
    "#define attribute in\n" \
    "#define varying out\n"
#define EVGRLU_BLUR_FS_HEADER \
    "#version 130\n" \
    "#define varying in\n" \
    "#define texture2D texture\n" \
    "out vec4 evgrlu_frag_out;\n" \
    "#define gl_FragColor evgrlu_frag_out\n"
#elif defined(EVGR_GLES3)
#define EVGRLU_BLUR_VS_HEADER \
    "#version 300 es\n" \
    "#define attribute in\n" \
...
#elif defined(EVGR_GLES2)
#define EVGRLU_BLUR_VS_HEADER ""
#define EVGRLU_BLUR_FS_HEADER "precision mediump float;\n"
#else // GL2
#define EVGRLU_BLUR_VS_HEADER ""
#define EVGRLU_BLUR_FS_HEADER ""
#endif
...
static const char * evgrlu__blur_vs_src =
    EVGRLU_BLUR_VS_HEADER
...
static const char * evgrlu__blur_fs_src =
    EVGRLU_BLUR_FS_HEADER
...
static const float evgrlu__blur_quad[] = {
    -1.0f, -1.0f, 0.0f, 0.0f,
    1.0f, -1.0f, 1.0f, 0.0f,
    -1.0f,  1.0f, 0.0f, 1.0f,
    1.0f,  1.0f, 1.0f, 1.0f,
...
static GLuint evgrlu__compile_shader(GLenum type, const char * src)
{
...
static int evgrlu__ensure_program(EVGRLUblurState * s)
{
    if(s->program_ok) return 1;
    if(s->program != 0) return 0;

    GLuint vs = evgrlu__compile_shader(GL_VERTEX_SHADER, evgrlu__blur_vs_src);
    GLuint fs = evgrlu__compile_shader(GL_FRAGMENT_SHADER, evgrlu__blur_fs_src);
    if(vs == 0 || fs == 0) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(evgrlu__blur_quad), evgrlu__blur_quad, GL_STATIC_DRAW);
...
static void evgrlu__ensure_scratch(EVGRLUblurState * s, EVGRcontext * ctx, int w, int h)
{
    if(s->scratch_fb && s->scratch_w >= w && s->scratch_h >= h) return;
    if(s->scratch_fb) {
        evgrluDeleteFramebuffer(s->scratch_fb);
        s->scratch_fb = NULL;
    }
    ...
    s->scratch_fb = evgrluCreateFramebuffer(ctx, alloc_w, alloc_h, 0, EVGR_TEXTURE_RGBA);
    s->scratch_w = s->scratch_fb ? alloc_w : 0;
    s->scratch_h = s->scratch_fb ? alloc_h : 0;
}

static void evgrlu__ensure_capture(EVGRLUblurState * s, int w, int h)
{
 ...
 * Returns tap_count. */
static int evgrlu__compute_kernel(int radius, int max_taps, int step_px,
                                 float * weights, float * offsets)
{
    ...
    /* Zero remaining */
    for(i = tap_count + 1; i <= EVGRLU_BLUR_MAX_TAPS; i++) {
        weights[i] = 0.0f;
        offsets[i] = 0.0f;
    }
    return tap_count;
...
static int evgrlu__compute_kernel_bilinear(int radius, int step_px,
                                          float * weights, float * offsets)
{
    float sigma = (float)radius / 3.0f;
    if(sigma < 1.0f) sigma = 1.0f;
    ...
    int n = radius < EVGRLU_BLUR_MAX_TAPS ? radius : EVGRLU_BLUR_MAX_TAPS;
    float dw[EVGRLU_BLUR_MAX_TAPS + 1];
    float total = 0.0f;
    int i;
    for(i = 0; i <= n; i++) {
        float x = (float)(i * step_px);
    ...
    /* Zero remaining */
    for(i = bi + 1; i <= EVGRLU_BLUR_MAX_TAPS; i++) {
        weights[i] = 0.0f;
        offsets[i] = 0.0f;
    }
    return bi;
...
{
#ifdef EVGR_FBO_VALID
    EVGRLUblurState * s = (EVGRLUblurState *)lv_malloc_zeroed(sizeof(EVGRLUblurState));
    return s;
#else
    return NULL;
#endif
...
void evgrluDeleteBlurState(EVGRLUblurState * state, EVGRcontext * ctx)
{
    ...
    EVGR_NOTUSED(ctx);
    if(!state) return;
    if(state->scratch_fb) evgrluDeleteFramebuffer(state->scratch_fb);
                    ...
                    const EVGRLUblurParams * params)
{
    ...
    if(radius > EVGRLU_BLUR_MAX_RADIUS) radius = EVGRLU_BLUR_MAX_RADIUS;

    if(!evgrlu__ensure_program(state)) return -1;
    evgrlu__ensure_scratch(state, ctx, w, h);
    if(!state->scratch_fb) return -1;

    int is_root = (fb == NULL);
    if(is_root) {
        evgrlu__ensure_capture(state, w, h);
        if(!state->capture_tex) return -1;
    }

    // Compute kernel — speed mode uses bilinear merging (half the fetches)
    float weights[EVGRLU_BLUR_MAX_TAPS + 1];
    float offsets[EVGRLU_BLUR_MAX_TAPS + 1];
    int tap_count;

    if(params->quality == EVGRLU_BLUR_QUALITY_SPEED) {
        int step_px = (radius + 8 - 1) / 8;
        if(step_px < 1) step_px = 1;
        tap_count = evgrlu__compute_kernel_bilinear(radius, step_px, weights, offsets);
    }
    else {
        int max_taps = EVGRLU_BLUR_MAX_TAPS;
        int step_px = (radius + max_taps - 1) / max_taps;
        if(step_px < 1) step_px = 1;
        tap_count = evgrlu__compute_kernel(radius, max_taps, step_px, weights, offsets);
    }
    ...
    glUniform1fv(state->loc_weights, EVGRLU_BLUR_MAX_TAPS + 1, weights);
    glUniform1fv(state->loc_offsets, EVGRLU_BLUR_MAX_TAPS + 1, offsets);
    ...
    if(is_root) {
        ...
        int iw = 0, ih = 0;
        evgrImageSize(ctx, fb->image, &iw, &ih);
        if(iw <= 0) iw = w;
        if(ih <= 0) ih = h;
        src_tex_w = iw;
        src_tex_h = ih;
    ...
    evgrluBindFramebuffer(state->scratch_fb);
    glViewport(0, 0, w, h);
